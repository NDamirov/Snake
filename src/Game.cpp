...
Game::Game(size_t field_w, size_t field_h)
        : snake(10, 10, field_w, field_h), field(field_w, field_h), interface(field_w, field_h), state(State::IN_PROGRESS), score(1) {
...
void Game::Run() {
    while(state == State::IN_PROGRESS) {
        ...
        while (std::chrono::duration<double>(std::chrono::system_clock::now() - start_time).count() < delay) {
            ParseKey();
        }
        MakeStep();
    }
